...
std::vector<std::array<double, 3>> ExtractWorkspaceTcpPositions(
    const std::vector<RoboSDP::Kinematics::Dto::WorkspacePointDto>& sampledPoints)
{
    ...
    {
        tcpPositions.push_back(point.tcp_pose.position_m);
    }
    return tcpPositions;
}

void ExtractWorkspaceTcpPositionsWithSingularity(
    const std::vector<RoboSDP::Kinematics::Dto::WorkspacePointDto>& sampledPoints,
...
void KinematicsWidget::OnSampleWorkspaceClicked()
{
    ...
    std::vector<std::array<double, 3>> tcpPositions;
    // 工作空间采样成功后，发射完整点云信号供 VTK 渲染。
    if (m_state.last_workspace_result.success)
    {
        tcpPositions = ExtractWorkspaceTcpPositions(m_state.last_workspace_result.sampled_points);
...
void KinematicsWidget::OnSingularityAnalysisClicked()
{
    ...
    if (result.success)
    {
        ...
        std::vector<std::array<double, 3>> tcpPositions;
        std::vector<bool> isSingular;
        ExtractWorkspaceTcpPositionsWithSingularity(
