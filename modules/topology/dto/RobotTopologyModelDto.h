 ...
 * 第一阶段仅支持模板化 6R 串联机器人，因此只保留最小必需字段。
 */
struct RobotDefinitionDto
{
    ...
    std::array<double, 2> j1_rotation_range_deg {-180.0, 180.0};
};
...
/**
 * @brief 肩肘腕布局与走线预留 DTO。
 * 保留一阶段最小可编辑字段，不提前扩展二期结构设计细节。
 */
struct TopologyLayoutDto
{
    QString shoulder_type = QStringLiteral("offset");
    QString elbow_type = QStringLiteral("articulated");
    QString wrist_type = QStringLiteral("spherical");
    bool wrist_intersection = true;
    bool wrist_offset = false;
