    ...
    const RoboSDP::Topology::Dto::RobotTopologyModelDto& model) const
{
    ...
    if (model.layout.shoulder_type.trimmed().isEmpty())
    {
        AddIssue(
            result,
            QStringLiteral("layout.shoulder_type"),
            QStringLiteral("TOPOLOGY_SHOULDER_TYPE_REQUIRED"),
            QStringLiteral("肩部形式不能为空。"));
    }

    if (model.layout.elbow_type.trimmed().isEmpty())
    {
        AddIssue(
            result,
            QStringLiteral("layout.elbow_type"),
            QStringLiteral("TOPOLOGY_ELBOW_TYPE_REQUIRED"),
            QStringLiteral("肘部形式不能为空。"));
    }

    if (model.layout.wrist_type.trimmed().isEmpty())
    {
        AddIssue(
            result,
            QStringLiteral("layout.wrist_type"),
            QStringLiteral("TOPOLOGY_WRIST_TYPE_REQUIRED"),
            QStringLiteral("腕部形式不能为空。"));
    }
