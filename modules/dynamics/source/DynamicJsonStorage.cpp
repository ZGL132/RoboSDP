...
RoboSDP::Kinematics::Dto::UnifiedRobotModelSnapshotDto FromUnifiedRobotSnapshotObject(const QJsonObject& object)
{
    ...
    snapshot.frame_semantics_version =
        ReadInt(object, QStringLiteral("frame_semantics_version"), snapshot.frame_semantics_version);
...
QJsonObject DynamicJsonStorage::ToModelJsonObject(const RoboSDP::Dynamics::Dto::DynamicModelDto& model) const
{
    ...
    metaObject.insert(QStringLiteral("unified_robot_model_ref"), model.meta.unified_robot_model_ref);
    metaObject.insert(QStringLiteral("kinematic_kernel_ready"), model.meta.kinematic_kernel_ready);
    metaObject.insert(
        QStringLiteral("kinematic_conversion_diagnostics"),
        model.meta.kinematic_conversion_diagnostics);
    metaObject.insert(
        QStringLiteral("unified_robot_snapshot"),
        ToUnifiedRobotSnapshotObject(model.meta.unified_robot_snapshot));
    ...
    RoboSDP::Dynamics::Dto::DynamicModelDto& model) const
{
    ...
    model.meta.unified_robot_model_ref =
        ReadString(metaObject, QStringLiteral("unified_robot_model_ref"));
    model.meta.kinematic_kernel_ready =
        metaObject.value(QStringLiteral("kinematic_kernel_ready")).toBool(false);
    model.meta.kinematic_conversion_diagnostics =
        ReadString(metaObject, QStringLiteral("kinematic_conversion_diagnostics"));
    model.meta.unified_robot_snapshot =
        FromUnifiedRobotSnapshotObject(metaObject.value(QStringLiteral("unified_robot_snapshot")).toObject());
    if (model.meta.unified_robot_snapshot.unified_robot_model_ref.trimmed().isEmpty())
    {
        model.meta.unified_robot_snapshot.unified_robot_model_ref = model.meta.unified_robot_model_ref;
        model.meta.unified_robot_snapshot.source_kinematic_id = model.meta.kinematic_ref;
        model.meta.unified_robot_snapshot.pinocchio_model_ready = model.meta.kinematic_kernel_ready;
        model.meta.unified_robot_snapshot.conversion_diagnostics =
            model.meta.kinematic_conversion_diagnostics;
        model.meta.unified_robot_snapshot.derived_artifact_relative_path =
            QStringLiteral("kinematics/derived/%1.urdf").arg(
                model.meta.kinematic_ref.trimmed().isEmpty()
                    ? QStringLiteral("unknown_kinematic")
                    : model.meta.kinematic_ref.trimmed());
        model.meta.unified_robot_snapshot.derived_artifact_version =
            QStringLiteral("legacy_snapshot_v%1").arg(model.meta.unified_robot_snapshot.snapshot_version);
        model.meta.unified_robot_snapshot.derived_artifact_state_code = QStringLiteral("legacy_unknown");
        model.meta.unified_robot_snapshot.derived_artifact_exists = false;
        model.meta.unified_robot_snapshot.derived_artifact_fresh = false;
    }
    if (model.meta.unified_robot_snapshot.derived_artifact_state_code.trimmed().isEmpty())
    {
        model.meta.unified_robot_snapshot.derived_artifact_relative_path =
            QStringLiteral("kinematics/derived/%1.urdf").arg(
                model.meta.kinematic_ref.trimmed().isEmpty()
