...
DynamicsBuildResult DynamicsService::BuildFromKinematics(const QString& projectRootPath) const
{
    ...
    result.message = QStringLiteral("已基于 KinematicModel 生成最小 DynamicModel。");
    return result;
}
    ...
    const RoboSDP::Kinematics::Dto::KinematicsWorkspaceStateDto& kinematicState) const
{
    ...
    model.installation_pose.position_m = kinematicState.current_model.base_frame.position_m;
    model.installation_pose.rpy_deg = kinematicState.current_model.base_frame.rpy_deg;

    model.links.clear();
    for (std::size_t index = 0; index < kinematicState.current_model.links.size(); ++index)
    {
        const auto& kinematicLink = kinematicState.current_model.links[index];
    ...
    {
        const auto& limit = kinematicState.current_model.joint_limits[index];
    ...
    model.end_effector.cog = {
        kinematicState.current_model.tcp_frame.translation_m[0],
        kinematicState.current_model.tcp_frame.translation_m[1],
        kinematicState.current_model.tcp_frame.translation_m[2] * 0.5};
